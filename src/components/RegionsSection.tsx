...
export const RegionsSection: React.FC = () => {
  ...
  return (
    <section id="regions" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-10">
          ...
          return (
            <button
              ...
              className={`px-4 py-2.5 rounded-lg text-xs sm:text-sm font-royal tracking-wider transition-all duration-300 flex items-center gap-2 ${
                isSelected
                  ? 'bg-gradient-to-r from-[#C9A85C] to-[#E8C87A] text-[#0C1421] font-bold shadow-[0_0_25px_rgba(201,168,92,0.3)] scale-105 border border-[#FFF0CA]'
                  : 'bg-[#142B4A]/80 hover:bg-[#1F4E79] text-[#D9DEE5] border border-[#C9A85C]/30'
              }`}
            >
              <MapPin className={`w-3.5 h-3.5 ${isSelected ? 'text-[#0C1421]' : 'text-[#C9A85C]'}`} />
        ...
        {isEditMode && (
          <button
            ...
            className="px-3.5 py-2.5 rounded-lg text-xs font-royal tracking-wider bg-[#1F4E79] hover:bg-[#2A75D3] text-[#FFF0CA] border border-[#C9A85C] flex items-center gap-1.5 shadow-lg transition-all"
          >
      ...
      {selectedRegion && (
        <OrnateFrame variant="gold" glow padding="p-6 sm:p-10" className="transition-all duration-500">
          
          {/* Header Banner */}
          <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-[#C9A85C]/30 pb-6 mb-8 gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded bg-[#142B4A] border border-[#C9A85C]/40 text-[11px] font-mono text-[#E8C87A] uppercase mb-2">
                <Compass className="w-3 h-3 text-[#C9A85C]" />
                <EditableText
              ...
              <h3 className="text-3xl sm:text-4xl font-extrabold font-royal text-[#FFF0CA] tracking-wide flex items-center gap-3">
                <EditableText
                  value={selectedRegion.name}
                  onSave={(val) => updateRegion(selectedRegion.id, { name: val })}
                  label="Бүсийн нэр"
                  className="text-3xl sm:text-4xl font-extrabold font-royal text-[#FFF0CA]"
                />
              ...
              {isEditMode && (
                <>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1F4E79] hover:bg-[#C9A85C] text-[#FFF0CA] hover:text-[#0C1421] rounded border border-[#C9A85C] text-xs font-royal transition-all shadow"
                  >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-8">
            ...
            <div className="region-card p-3 rounded text-center">
              <span className="text-[10px] uppercase font-serif tracking-wider text-[#C9A85C] block mb-1">Гарал үүсэл</span>
              <div className="royal-font text-xs sm:text-sm font-bold text-[#FFF0CA]">
                <EditableText
            ...
            {/* 2. Area */}
            <div className="region-card p-3 rounded text-center">
              <span className="text-[10px] uppercase font-serif tracking-wider text-[#C9A85C] block mb-1">Газар нутаг</span>
            ...
            {/* 3. Leading House */}
            <div className="region-card p-3 rounded text-center">
              <span className="text-[10px] uppercase font-serif tracking-wider text-[#C9A85C] block mb-1">Толгой овог</span>
              <div className="royal-font text-xs sm:text-sm font-bold text-[#FFF0CA]">
                <EditableText
            ...
            {/* 4. Leader */}
            <div className="region-card p-3 rounded text-center bg-gradient-to-b from-[#142B4A]/90 to-[#0C1421] border-[#C9A85C]/50">
              <span className="text-[10px] uppercase font-serif tracking-wider text-[#E8C87A] font-bold block mb-1">Газрын тэргүүн</span>
              <div className="royal-font text-xs sm:text-sm font-extrabold text-[#FFF0CA]">
                <EditableText
            ...
            <div className="region-card p-3 rounded text-center">
              <span className="text-[10px] uppercase font-serif tracking-wider text-[#C9A85C] block mb-1">Төвлөрсөн овог</span>
            ...
            {/* 6. Tax */}
            <div className="region-card p-3 rounded text-center">
              <span className="text-[10px] uppercase font-serif tracking-wider text-[#C9A85C] block mb-1">Татвар</span>
              <div className="font-mono text-xs sm:text-sm font-bold text-[#E8C87A]">
                <EditableText
          ...
          <div className="border-t border-[#C9A85C]/30 pt-6">
            <div className="flex flex-wrap items-center gap-2 mb-6">
              <button
                ...
                className={`px-3.5 py-1.5 rounded text-xs font-serif tracking-wider uppercase transition-all ${
                  activeTab === 'overview'
                    ? 'bg-[#C9A85C] text-[#0C1421] font-bold'
            ...
            {activeTab === 'overview' && (
              <div className="space-y-4">
                <div className="text-xs uppercase font-serif tracking-wider text-[#C9A85C] flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {selectedRegion.mainBuildings?.map((bldg, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-[#0C1421]/90 rounded border border-[#1F4E79]/40 hover:border-[#C9A85C]/50 flex items-center gap-2.5 text-xs text-[#FFF0CA] font-royal shadow-sm"
                    >
            ...
            {activeTab === 'nature' && selectedRegion.naturalFeatures && (
              <div className="space-y-4">
                <div className="text-xs uppercase font-serif tracking-wider text-[#C9A85C] flex items-center gap-2">
                  <Trees className="w-4 h-4 text-[#C9A85C]" />
            ...
            {activeTab === 'products' && selectedRegion.mainProducts && (
              <div className="space-y-4">
                <div className="text-xs uppercase font-serif tracking-wider text-[#C9A85C] flex items-center gap-2">
                  <Gem className="w-4 h-4 text-[#C9A85C]" />
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRegion.mainProducts.map((prod, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1.5 rounded-full bg-[#142B4A]/70 border border-[#C9A85C]/40 text-xs font-sans text-[#FFF0CA] shadow-sm"
                    >
                      ✦ {prod}
                    </span>
                  ))}
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRegion.estateHolders.map((holder, idx) => (
                    <div
                      key={idx}
                      className="px-3 py-1.5 bg-[#0C1421] border border-[#C9A85C]/50 rounded text-xs font-royal text-[#E8C87A]"
                    >
                      👑 {holder}
                    </div>
                  ))}
            ...
            {selectedRegion.historicalNotes && (
              <div className="mt-8 pt-4 border-t border-[#C9A85C]/20 text-xs text-[#D9DEE5]/80 font-serif italic">
                <span className="text-[#C9A85C] not-italic font-bold mr-2">Түүхэн тэмдэглэл:</span>
                <EditableText
