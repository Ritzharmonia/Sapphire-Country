...
const ROYAL_PRESETS = [
  ...
  {
    id: 'preset-crest-gold-lion',
    title: 'Алтан Арслан Сүлд',
    category: 'crest',
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500" width="100%" height="100%">
  ...
  <circle cx="250" cy="250" r="220" fill="none" stroke="url(#goldG)" stroke-width="4"/>
  <circle cx="250" cy="250" r="205" fill="none" stroke="url(#goldG)" stroke-width="1.5" stroke-dasharray="8,6"/>
  <!-- Crown Top -->
  <path d="M 180 140 L 205 80 L 230 115 L 250 60 L 270 115 L 295 80 L 320 140 Z" fill="url(#goldG)" stroke="#5A4315" stroke-width="2"/>
  <circle cx="250" cy="55" r="8" fill="#FFF0CA"/>
  <!-- Shield Body -->
  <path d="M 160 160 L 340 160 Q 340 320 250 410 Q 160 320 160 160 Z" fill="#142B4A" stroke="url(#goldG)" stroke-width="6"/>
  ...
  <text x="250" y="445" text-anchor="middle" fill="#E8C87A" font-family="'Cormorant Garamond', serif" font-size="14" letter-spacing="4" font-weight="bold">COUNTRY OF SAPPHIRE</text>
</svg>
`)}`
  },
  {
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 600" width="100%" height="100%">
  <defs>
    ...
    <linearGradient id="queenGold" x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <stop offset="100%" stop-color="#A27D2C"/>
    </linearGradient>
  </defs>
  <rect width="500" height="600" fill="url(#queenGlow)"/>
  <rect x="25" y="25" width="450" height="550" rx="16" fill="none" stroke="url(#queenGold)" stroke-width="2" opacity="0.85"/>
  <circle cx="250" cy="220" r="145" fill="none" stroke="url(#queenGold)" stroke-width="1.5" opacity="0.4"/>
  <!-- Robe -->
  <path d="M 130 560 L 165 350 Q 250 310 335 350 L 370 560 Z" fill="#142B4A" stroke="url(#queenGold)" stroke-width="2"/>
  <path d="M 210 340 Q 250 450 290 340 Z" fill="#0C1421" stroke="url(#queenGold)" stroke-width="1"/>
  <!-- Head Silhouette -->
  <circle cx="250" cy="220" r="52" fill="#1B3A61" stroke="url(#queenGold)" stroke-width="1.5"/>
  <path d="M 210 230 Q 250 290 290 230 Q 270 310 250 320 Q 230 310 210 230 Z" fill="#142B4A" stroke="url(#queenGold)" stroke-width="1"/>
  <!-- Tiara -->
  <path d="M 205 160 L 225 125 L 250 100 L 275 125 L 295 160 Z" fill="url(#queenGold)" stroke="#8A6D2B" stroke-width="1.5"/>
  ...
  <text x="250" y="530" text-anchor="middle" fill="#E8C87A" font-family="'Cormorant Garamond', serif" font-size="16" letter-spacing="4" font-weight="bold">HER MAJESTY THE QUEEN</text>
...
export const ImageManagerModal: React.FC = () => {
  ...
  // Process and compress image file to ensure fast performance and no localstorage overflows
  const processFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      // If raster image, compress via Canvas to WebP / JPEG (800x800 max)
      const img = new Image();
      img.onerror = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-2xl">
        <OrnateFrame variant="gold" glow padding="p-5 sm:p-7" className="max-h-[92vh] overflow-y-auto">
          
          {/* Header */}
          <div className="flex items-center justify-between border-b border-[#C9A85C]/30 pb-3 mb-5">
            <div className="flex items-center gap-2.5">
              {isMonarch ? (
                <Crown className="w-5 h-5 text-[#E8C87A]" />
              ) : isCrest ? (
                <Shield className="w-5 h-5 text-[#C9A85C]" />
              ) : (
                <ImageIcon className="w-5 h-5 text-[#2A75D3]" />
              )}
              <h3 className="font-royal text-base sm:text-lg font-bold text-[#FFF0CA]">
                {modalTitle}
              </h3>
            </div>
            <button
              type="button"
              onClick={closeModal}
              className="p-1.5 text-[#D9DEE5]/60 hover:text-[#FFF0CA] rounded-full hover:bg-[#142B4A] transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="flex items-center justify-between w-full mb-2">
                <span className="text-[10px] sm:text-[11px] uppercase font-serif text-[#C9A85C] tracking-wider">
              ...
              <div className={`relative w-48 h-48 sm:w-56 sm:h-56 ${isMonarch ? 'rounded-2xl' : 'rounded-xl'} bg-[#0C1421] border-2 border-[#C9A85C]/80 flex items-center justify-center overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.8),0_0_15px_rgba(201,168,92,0.15)]`}>
                {imageUrl ? (
                  ...
                  <div className="text-center p-4 text-xs text-[#D9DEE5]/60 font-serif flex flex-col items-center gap-1.5">
                    <ImageIcon className="w-8 h-8 text-[#C9A85C]/40" />
                    <span>Зураг оруулаагүй байна</span>
                  </div>
                )}

                {/* Corner Ornate Accents */}
                <div className="absolute top-1.5 left-1.5 w-3 h-3 border-t-2 border-l-2 border-[#C9A85C]/70 pointer-events-none" />
                <div className="absolute top-1.5 right-1.5 w-3 h-3 border-t-2 border-r-2 border-[#C9A85C]/70 pointer-events-none" />
                <div className="absolute bottom-1.5 left-1.5 w-3 h-3 border-b-2 border-l-2 border-[#C9A85C]/70 pointer-events-none" />
                <div className="absolute bottom-1.5 right-1.5 w-3 h-3 border-b-2 border-r-2 border-[#C9A85C]/70 pointer-events-none" />
              </div>
              ...
              <div className="flex items-center gap-2 mt-3 w-full justify-center">
                <button
                  ...
                  className="px-3 py-1 rounded bg-[#142B4A] text-[11px] font-serif text-[#D9DEE5] hover:text-[#FFF0CA] border border-[#C9A85C]/30 flex items-center gap-1.5 transition-colors shadow"
                >
                  <RotateCcw className="w-3 h-3 text-[#C9A85C]" />
            ...
            <div className="space-y-3.5 text-xs flex flex-col justify-between">
              ...
              {/* Tabs */}
              <div>
                <div className="flex rounded-md overflow-hidden border border-[#C9A85C]/40 bg-[#0C1421] mb-3">
                  <button
                    type="button"
                    onClick={() => setTab('upload')}
                    className={`flex-1 py-1.5 font-serif text-[11px] transition-colors flex items-center justify-center gap-1.5 ${
                      tab === 'upload' ? 'bg-[#C9A85C] text-[#0C1421] font-bold shadow' : 'text-[#D9DEE5] hover:bg-[#142B4A]'
                    }`}
                  >
                  ...
                  <button
                    ...
                    onClick={() => setTab('url')}
                    className={`flex-1 py-1.5 font-serif text-[11px] transition-colors flex items-center justify-center gap-1.5 ${
                      tab === 'url' ? 'bg-[#C9A85C] text-[#0C1421] font-bold shadow' : 'text-[#D9DEE5] hover:bg-[#142B4A]'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 font-serif text-[11px] transition-colors flex items-center justify-center gap-1.5 ${
                      tab === 'presets' ? 'bg-[#C9A85C] text-[#0C1421] font-bold shadow' : 'text-[#D9DEE5] hover:bg-[#142B4A]'
                    }`}
                  >
                  ...
                  <div>
                    <div 
                      ...
                      className={`p-4 border-2 border-dashed rounded-lg text-center cursor-pointer transition-all group ${
                        isDragging 
                          ? 'border-[#FFF0CA] bg-[#1F4E79]/80 scale-[1.02]' 
                          : 'border-[#C9A85C]/60 hover:border-[#E8C87A] bg-[#0C1421]/80 hover:bg-[#142B4A]/50'
                      }`}
                    >
                      <UploadCloud className="w-8 h-8 text-[#C9A85C] group-hover:text-[#FFF0CA] mx-auto mb-1.5 transition-transform group-hover:scale-110" />
                      <p className="text-xs text-[#FFF0CA] font-serif mb-1 font-bold">
                      ...
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-[#1F4E79] hover:bg-[#2A75D3] text-[#FFF0CA] rounded text-xs font-royal border border-[#C9A85C] shadow inline-flex items-center gap-1.5"
                      >
                        <Camera className="w-3.5 h-3.5 text-[#E8C87A]" />
                ...
                {/* Tab 2: URL Input */}
                {tab === 'url' && (
                  <div className="space-y-2 bg-[#0C1421]/80 p-3 rounded border border-[#C9A85C]/30">
                    <label className="block text-[11px] font-serif text-[#C9A85C] uppercase">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-[#142B4A]/80 text-[#FFF0CA] border border-[#C9A85C]/50 rounded px-3 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#C9A85C]"
                      />
                      <button
                        type="button"
                        onClick={handleUrlApply}
                        className="px-3 py-1.5 bg-[#1F4E79] hover:bg-[#2A75D3] text-[#FFF0CA] rounded border border-[#C9A85C]/50 font-serif text-xs shrink-0"
                      >
                        Шалгах
                      </button>
                    </div>
                      ...
                      {ROYAL_PRESETS.map((p) => (
                        <button
                          ...
                          className={`p-2 rounded border text-left flex items-center gap-2 transition-all ${
                            imageUrl === p.url 
                              ? 'bg-[#1F4E79] border-[#E8C87A] ring-1 ring-[#E8C87A]' 
                              : 'bg-[#0C1421] hover:bg-[#142B4A] border-[#C9A85C]/40'
                          }`}
                        >
                          {p.category === 'monarch' ? (
                            <Crown className="w-4 h-4 text-[#E8C87A] shrink-0" />
                          ) : (
                            <Shield className="w-4 h-4 text-[#C9A85C] shrink-0" />
                          )}
                          <div className="overflow-hidden">
                            <div className="text-[11px] font-royal font-bold text-[#FFF0CA] truncate">{p.title}</div>
              ...
              <div className="space-y-2.5 pt-3 border-t border-[#C9A85C]/20 bg-[#0C1421]/60 p-2.5 rounded">
                <div>
                  <div className="flex justify-between text-[10px] sm:text-[11px] font-serif text-[#C9A85C] mb-1">
                    <span className="flex items-center gap-1">
                      <ZoomIn className="w-3 h-3" />
                      <span>Томруулах / Багасгах (Scale):</span>
                    </span>
                    <span className="font-mono text-[#FFF0CA]">{scale.toFixed(2)}x</span>
                  </div>
                  <input
                    type="range"
                    min="0.3"
                    ...
                    className="w-full accent-[#C9A85C] cursor-pointer"
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="flex justify-between text-[10px] font-serif text-[#C9A85C] mb-1">
                      <span>Хэвтээ (X):</span>
                      <span className="font-mono text-[#FFF0CA]">{offsetX}px</span>
                    </div>
                    <input
                      type="range"
                      min="-150"
                  ...
                  <div>
                    <div className="flex justify-between text-[10px] font-serif text-[#C9A85C] mb-1">
                      <span>Босоо (Y):</span>
                      <span className="font-mono text-[#FFF0CA]">{offsetY}px</span>
                    </div>
                    <input
                      type="range"
                      min="-150"
          ...
          {/* Footer Buttons */}
          <div className="flex items-center justify-between mt-5 pt-3 border-t border-[#C9A85C]/30">
            <span className="text-[11px] text-[#E8C87A] font-mono">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                type="button"
                onClick={closeModal}
                className="px-3.5 py-1.5 bg-[#142B4A] hover:bg-[#1F4E79] text-[#D9DEE5] rounded text-xs font-royal border border-[#C9A85C]/30 transition-colors"
              >
                Болих (Cancel)
              </button>
              <button
                type="button"
                onClick={handleSave}
                className="px-5 py-2 bg-[#C9A85C] hover:bg-[#E8C87A] text-[#0C1421] font-bold rounded text-xs font-royal flex items-center gap-1.5 shadow-lg transform hover:scale-105 transition-all cursor-pointer"
              >
